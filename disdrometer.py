...
if __name__ == "__main__":
    ...
    files = '../EPIP/sgpldE13.b1/sgpldE13.b1.20190520.000000.cdf'

    obj = xr.open_dataset(files)
    drop_diams = obj['median_volume_diameter'].values
    fall_speed = obj['fall_velocity_calculated'].values
    drop_times = obj['time'].values

    bins = np.arange(0, 10.2, 0.2)
    psd = obj['number_density_drops'].values
    area = obj['moment2'].values
    volume = obj['moment3'].values
    ...
    i = 0
    rain_rate = np.zeros_like(obj['rain_rate'].values)
    for times in obj['time'].values:
    ...
    print(rain_rate)
    #old_attrs_nd = obj['num_drops'].attrs
